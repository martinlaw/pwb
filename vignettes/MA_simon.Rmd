...
ma.simon.plot.comb <- ggplot(data=ma.simon.results.comb, mapping=aes(x=theta, y=bias, col=type))+
  ...
  labs(x="True response probability theta",
       y="Bias (estimate - theta)",
       title="Bias in meta-analysis (Simon design)")+
  geom_hline(aes(yintercept=0), col="grey", linetype="dashed")+
  theme(legend.position="bottom")+
...
ma.simon.results.comb.tim <- rbind(ma.simon.results.tim, ma.simon.results2.tim, ma.simon.results3.tim)
#ma.simon.results0$p0p1 <- bquote(p[0]==0.5~","~p[1]==0.6)
...
```{r "simon_MA_Tim", fig.height=4, fig.width=4, echo=FALSE, warning=T}
library(ggplot2)
ma.simon.plot.tim <- ggplot(data=ma.simon.results.tim, mapping=aes(x=theta, y=bias, col=type))+
  geom_line(alpha=0.4, size=1)+
  geom_point()+
  labs(x="True response probability theta",
       y="Bias (estimate - theta)",
  ...
  geom_vline(aes(xintercept=0.6), col="grey", linetype="dashed")+
  geom_hline(aes(yintercept=0), col="grey", linetype="dashed")+
  ylim(max(abs(ma.simon.results.tim$bias)) * c(-1.1, 1.1) )+
  scale_x_continuous(breaks=theta.vec.0.5)+
  theme(legend.position="bottom")
ma.simon.plot.tim
```
