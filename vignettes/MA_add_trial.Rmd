...
#colnames(combined.results2$theta.hat)[1] <- "Simon"
...
```{r precision}
emp.SE <-numeric(length = ncol(combined.results2$theta.hat))
for(i in 1:ncol(combined.results2$theta.hat)){
  emp.SE[i] <- empSE(combined.results2$theta.hat[, i])
}
...
# Precision for Simon design is `prec.simon`.
