...
```{r "simon", echo=FALSE, fig.height=4, fig.width=4, warning=TRUE}
...
```{r "simon_subset", echo=FALSE, fig.height=4, fig.width=4, warning=TRUE}
...
simon.plot.sub <- ggplot(data=simon.sub.df, mapping=aes(x=theta, y=bias, col=type))+  geom_line(alpha=0.4, size=1)+
  ...
  labs(x="True response probability theta",
       y="Bias",
       title="Bias in Simon design (p0=0.5, p1=0.6)",
       subtitle="Includes proportion of trials stopped early")+
...
```{r "simon2_subset", fig.height=4, fig.width=4, echo=FALSE, warning=T}
#### Plot bias -- no subsetting of "stopped early" or "stopped at N" ####
...
odd <- seq(from=1, to=length(theta.vec2), by=2)
simon.plot.sub2 <- ggplot(data=simon.sub.df2, mapping=aes(x=theta, y=bias, col=type))+
  ...
  labs(x="True response probability theta",
       ...
       subtitle="Includes proportion of trials stopped early")+
  annotate("text", x=theta.vec2[odd], y=-0.025, label=round(stop.early.count2[odd],2))+
