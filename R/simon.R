...
pwbSimon <- function(p, des, nsims=1e5){
  ...
  for(i in 1:nsims){
    ...
    onetrial <- rbinom(n=N, size=1, prob=p) # Simulate one trial of N results
...
states.data[[i+1]] <- DHARMa::getRandomState(NULL)
  ...
  ### What happens when there is no correction for 
...
##### Obtain bias for range of true reponse probabilities: ####
